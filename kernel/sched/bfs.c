...
/*
 * RQ niffy is an increasing nanosecond counter.
 */
...
static inline void grq_test_lock(raw_spinlock_t *lock)
{
	if (lock != &grq.lock)
		raw_spin_lock(&grq.lock);
}

static inline void grq_test_unlock(raw_spinlock_t *lock)
{
	if (lock != &grq.lock)
		raw_spin_unlock(&grq.lock);
}
...
/*
static inline struct rq
*task_vrq_lock_irq(struct task_struct *p, raw_spinlock_t **plock)
{
	struct rq *rq;
	for (;;) {
		rq = task_rq(p);
...
static inline void
task_vrq_unlock_irq(raw_spinlock_t *lock)
{
	raw_spin_unlock(&grq.lock);
 ...
 *
 * Same cpu as task originally runs on
 * SMT of the cup
 * Cores/threads shares last level cache
 * Cores within the same physical cpu
 * Cpus/Cores within the local NODE
 * All cpus avariable
 * (current no scaling is considered)
 */
 ...
 */
static inline int closest_mask_cpu(int cpu, cpumask_t *cpumask)
{
	cpumask_t tmpmask;
	...
	/* SMT of the cpu */
#ifdef CONFIG_X86
	if (cpumask_and(&tmpmask, cpumask, cpu_sibling_mask(cpu)))
#else
	if (cpumask_and(&tmpmask, cpumask, cpu_smt_mask(cpu)))
#endif
		return cpumask_first(&tmpmask);
#endif

#ifdef CONFIG_SCHED_MC

	/* Cores shares last level cache */
	if (cpumask_and(&tmpmask, cpumask, cpu_coregroup_mask(cpu)))
		return cpumask_first(&tmpmask);

#ifdef CONFIG_X86
	/* Cores within the same physical cpu */
	if( cpumask_and(&tmpmask, cpumask, cpu_core_mask(cpu)))
		return cpumask_first(&tmpmask);
#endif
#endif

	/* Cpus/Cores within the local NODE */
	if( cpumask_and(&tmpmask, cpumask, cpu_cpu_mask(cpu)))
		return cpumask_first(&tmpmask);
...
static inline int best_mask_cpu(struct rq *rq, cpumask_t *cpumask)
{
	int cpu = rq->cpu;
	...
	return closest_mask_cpu(cpu, cpumask);
}
...
static inline bool resched_best_idle(struct task_struct *p)
{
	...
	if (cpumask_and(&check_cpumask, &p->cpus_allowed, &grq.cpu_idle_map)) {
		...
		best_cpu = best_mask_cpu(task_rq(p), &check_cpumask);
#ifdef CONFIG_SMT_NICE
...
resched_closest_idle(struct rq *rq, int cpu, struct task_struct *p)
{
	...
	if (cpumask_and(&check_cpumask, &p->cpus_allowed, &grq.cpu_idle_map)) {
		...
		best_cpu = closest_mask_cpu(cpu, &check_cpumask);
#ifdef CONFIG_SMT_NICE
		if (!smt_should_schedule(p, best_cpu))
			return false;
#endif
...
static void time_slice_expired(struct task_struct *p, struct rq *rq)
{
	...
	p->deadline = rq->niffy + task_deadline_diff(p);
#ifdef CONFIG_SMT_NICE
	if (!p->mm)
		p->smt_bias = 0;
	else if (rt_task(p))
				...
				unsigned long *switch_count)
{
	...
	if (likely(queued_notrunning())) {
		...
		if (likely(prev != next)) {
			...
			context_switch(rq, prev, next); /* unlocks the grq&rq */
...
int task_prio(const struct task_struct *p)
{
	...
	delta = NS_TO_MS(p->deadline - this_rq()->niffy);
